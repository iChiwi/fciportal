...
if (!file_exists($fullPath)) {
    $debug['mkdir'] = "Creating directory: $fullPath";
    $mkdirResult = mkdir($fullPath, 0755, true);
    $debug['mkdirResult'] = $mkdirResult ? 'success' : 'failed';
    
    if (!$mkdirResult) {
        $debug['mkdir_error'] = error_get_last();
        header('Content-Type: application/json');
        echo json_encode([
            'error' => 'Failed to access or create directory',
            'debug' => $debug
        ]);
        exit();
    }
...
try {
    if (is_dir($fullPath)) {
        ...
        $debug['is_dir'] = false;
        $debug['error'] = "Not a valid directory";
    }
} catch (Exception $e) {
